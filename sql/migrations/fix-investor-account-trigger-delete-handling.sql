-- Fix investor account balance drift when completed capital transactions are deleted.
-- Existing trigger logic handled INSERT/UPDATE deltas but did not reverse DELETE events.
...
BEGIN
  -- INSERT: apply NEW completed effect once.
  ...
  -- DELETE: reverse OLD completed effect.
  ...
  -- UPDATE: reverse OLD completed effect first, then apply NEW completed effect.
...
DROP TRIGGER IF EXISTS trigger_update_investor_balances ON capital_transactions;
CREATE TRIGGER trigger_update_investor_balances
  AFTER INSERT OR UPDATE OR DELETE ON capital_transactions
  FOR EACH ROW
  EXECUTE FUNCTION update_investor_account_balances();

-- One-time reconciliation: source-of-truth is completed rows in capital_transactions.
WITH tx_summary AS (
  SELECT
    investor_id,
